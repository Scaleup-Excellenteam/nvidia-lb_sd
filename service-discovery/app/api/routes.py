...
router = APIRouter()
log = getLogger("Service-Discovery.API")

# ---- UI status Literal types (match your Pydantic model fields) ----
UiPartStatus = Literal["healthy", "warning", "error"]
UiEndpointStatus = Literal["running", "stopped"]


# ------- helpers to map internal -> UI strings -------
def _part_status_to_ui(s: Status) -> UiPartStatus:
    # UP -> healthy, DEGRADED -> warning, DOWN -> error
    if s is Status.UP:
        return "healthy"
    if s is Status.DEGRADED:
        return "warning"
    return "error"


def _endpoint_status_to_ui(s: Status) -> UiEndpointStatus:
    # UP -> running, otherwise -> stopped
    return "running" if s is Status.UP else "stopped"
...
async def register_endpoint(ep: EndpointIn):
    ...
    log.info("[SD] POST /endpoints — creates/updates an endpoint; returns the saved endpoint JSON")
    return registry.upsert(ep)


@router.delete("/endpoints/{endpoint_id}")
async def deregister_endpoint(endpoint_id: str):
    log.info("[SD] DELETE /endpoints/{id} — removes an endpoint; returns {'ok': True} if it existed")
    if not registry.deregister(endpoint_id):
        log.warning("[SD] Endpoint %s not found for deletion", endpoint_id)
...
async def set_status(endpoint_id: str, status: Status = Query(...)):
    log.info("[SD] PUT /endpoints/{id}/status — sets status; returns the endpoint with new status")
...
async def heartbeat(endpoint_id: str):
    log.info("[SD] POST /endpoints/{id}/heartbeat — marks alive; returns the endpoint with refreshed last_heartbeat")
...
async def list_endpoints(image_id: str, healthy: bool = True):
    log.info("[SD] GET /images/{image}/endpoints — lists endpoints; returns a list (maybe empty)")
    return registry.list_by_image(image_id, healthy_only=healthy)


@router.get("/services-map")
async def services_map():
    log.info("[SD] GET /services-map — shows catalog; returns image_id -> list of endpoints")
...
async def register_part(part: SystemPartIn):
    log.info("[SD] POST /parts — creates/updates a system part; returns the saved part JSON")
...
async def deregister_part(part_id: str):
    log.info("[SD] DELETE /parts/{id} — removes a part; returns {'ok': True} if it existed")
...
async def update_part_status(part_id: str, status: Status = Query(...)):
    log.info("[SD] PUT /parts/{id}/status — sets status; returns the part with new status")
...
async def heartbeat_part(part_id: str):
    log.info("[SD] POST /parts/{id}/heartbeat — marks alive; returns the part with refreshed last_heartbeat")
...
async def list_parts_by_kind(kind: str, healthy: bool = True):
    log.info("[SD] GET /parts/{kind} — lists parts of this kind; returns a list (maybe empty)")
...
async def list_all_parts(healthy: bool = True):
    log.info("[SD] GET /parts — lists all parts; returns a list (maybe empty)")
...
async def parts_map():
    log.info("[SD] GET /parts-map — shows catalog; returns kind -> list of parts")
...
async def services_flat():
    ...
    log.info("[SD] GET /services — lists all services; returns a list (maybe empty)")
    return [
        ServiceItem(
            id=p.id,
            name=p.kind,
...
async def containers_flat(image_id: str = Query(..., description="image id to list")):
    ...
    log.info("[SD] GET /containers — lists all containers for image_id=%s; returns a list (maybe empty)", image_id)
    return [
        ContainerItem(
            id=e.id,
            image_id=e.image_id,
